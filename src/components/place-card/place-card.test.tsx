...
describe('Component: PlaceCard', () => {
  it('Ожидаю выполнения запроса если пользователь нажимает кнопку добавить в избранное',
    async () => {
      ...
      const { withStoreComponent, mockStore, mockAxiosAdapter } =
        withStore(withHistoryComponent, makeFakeStore({}));
      mockAxiosAdapter.onPost(`${APIRoute.Favorites}/${offerId}/${status}`)
        .reply(201, mockFullOffer);
    ...
    () => {
      ...
      const waitingRenderTimer = setTimeout(() => {
        ...
        expect(screen.getByRole('button'))
          .toHaveClass(notExpectedFavoritesClass);
  ...
  it('Ожидаю отрисовку компонента если предложение премиальное и в избранном', () => {
    ...
    const waitingRenderTimer = setTimeout(() => {
      ...
      expect(screen.getByRole('button'))
        .toHaveClass(expectedFavoritesClass);
      expect(screen.getByText(mockOffer.title)).toBeInTheDocument();
      expect(screen.getByText(mockOffer.price)).toBeInTheDocument();
      expect(screen.getByText(mockOffer.type)).toBeInTheDocument();
      clearTimeout(waitingRenderTimer);
  ...
  it('Ожидаю перехода на страницу предложения', () => {
    ...
    const withHistoryComponent =
      withHistory(<PlaceCard offer={mockOffer} type='cities' />, mockHistory);
